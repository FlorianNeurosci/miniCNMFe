...
`detrend_movie` (`minicnmfe/detrend.py`) addresses *temporal* drift (bleach,
...
1. **Flat-field divide before greedy init.** Compute the per-pixel
   ...
   `minicnmfe/flatfield.py` and wire as an optional preprocessing step in
...
3. **Local-adaptive `min_corr` / `min_pnr`.** Replace the scalar
   ...
   quantiles inside `minicnmfe/initialization.py`. Real fix, but a pipeline
