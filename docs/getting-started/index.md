...
**Note:** On Windows, `n_jobs != 1` uses the `loky` backend (spawn-based). First call has overhead (~1–2 s) to start workers. Worthwhile for movies with T > 500 or large H×W.
...
(`--params tuning/tune_*/recommended_params.json`). See [parameter tuning](../tuning/index.md) for
the full guide.
...
> **Tip:** To get a starting point automatically — recommended values plus graphs to
> judge them — run the [parameter tuning](../tuning/index.md) workflow (`python tune.py <path>`)
...
### Bayesian prior on `g` via `decay_time_ms` and `frame_rate_hz`
...
> **Note:** Tests use a synthetic movie (32×32 pixels, 150 frames, 3 neurons) generated by `tests/conftest.make_synthetic_movie()`. Tests run in a few seconds on any machine.
...
- Lower `merge_thr_corr` (try `0.85` → `0.75`) so near-duplicate components with high temporal correlation fuse
...
> **Note:** `seed_suppress_factor` is a **deprecated no-op** and no longer has any effect — greedy init suppresses already-found neurons with an `ind_search` support mask, not a suppression disk. Do not reach for it to fix over-detection; tune the merge knobs above instead.
...
The AR coefficient `g` is biased upward — typically because the ring background under-subtracted slow drift and Yule-Walker reads it as a long calcium tail. Check `model.g[0][0]`: if it's near 0.96 (the default `fudge_factor` ceiling), `g` has pinned. Fix by setting `decay_time_ms` + `frame_rate_hz` on `CNMFeParams` (see [Bayesian prior on g via decay_time_ms and frame_rate_hz](#bayesian-prior-on-g-via-decay_time_ms-and-frame_rate_hz)). For 8m at 20 Hz, `g_target ≈ 0.76`.
